  ...
  return (
    <div className="h-full flex flex-col relative text-white">
      {card.type === "video" && !showPostVideoContent && (
        <div className="w-full h-full absolute inset-0 z-0" onClick={toggleVideoPlay}>
          <video
            ref={videoRef}
            src={card.videoUrl}
        ...
        <div className="flex-1 flex flex-col items-center mt-14">
              ...
              {card.showTimerAfterVideo && (
                <div className="flex-col">
                  <div className="flex flex-col space-y-4 mb-12">
                    {card.title && (
                      <h1 className="text-2xl font-medium text-white leading-tight">{card.title}</h1>
                    )}
          ...
          {card.type === "input" && (
            <div className="flex flex-col w-full h-full space-y-4 pointer-events-auto">
              ...
              {card.content && <p className="text-white">{card.content}</p>}
              <TextArea value={userInput} onChange={(e) => onUserInputChange(e.target.value)} />
            </div>
          )}
        </div>

        {index < cardsLength - 1 && card.type === "video" && !videoEnded && (
          <div className="absolute bottom-[4.125rem] left-0 right-0 flex justify-center z-20">
            <SwipeIcon />
          </div>
        )}
      </div>
      ...
      {((card.type === "input" && hasTyped) ||
        (card.type === "video" && card.showInputAfterVideo && videoEnded && hasTyped)) &&
      isActive &&
      mounted
        ? createPortal(
            <div className="fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom,0px)+18px)] z-[1000]">
              <div className="mx-auto w-full max-w-md px-4">
                <Button onClick={handleSubmit} variant="button" aria-label="Submit" className="w-full">
                  Submit
                </Button>
              </div>
            </div>,
